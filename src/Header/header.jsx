...
export default function header(props) {
  ...
  const { handleChangeSearch, toggleTheme ,pathToPage, searchItems, isDarkMode } =
  ...
  return (
    <div id="myHeader" className=" w-screen  h-auto z-20">
      <div id="header-1" className="w-screen">
        <div className="">
          <div
            className={` w-full h-16 text-2xl flex items-center gap-3 flex-row justify-between  ${
              isDarkMode ? "bg-slate-900 text-white" : "bg-slate-200 text-black"
            }  px-6`}
          >
            <div
              className={`flex items-center gap-3 flex-row  justify-start ${
                isDarkMode
                  ? "bg-slate-900 text-white"
                  : "bg-slate-200 text-black"
              } `}
            >
              <a href="/aaminshop/" className="text-2xl font-bold flex">
                <img
                  src="./assets/logo1.png"
                  className={`${
                    isDarkMode ? "" : "bg-black rounded-full"
                  } h-12 w-12`}
                  alt=""
                />
            ...
            <div className="flex items-center gap-3 flex-row justify-start ">
              <p className="group SearchButton text-2xl cursor-pointer flex flex-row items-center justify-start">
                <input
                  ...
                  className="SearchInput text-sm group-hover:px-3 text-black sm:w-56 md:w-64 rounded-xl h-6"
                />
              ...
              <p
                ...
                onClick={() => {
                  navigate("/aaminshop/profile");
                }}
              >
                <FaUserAlt />
              </p>
              <p onClick={toggleTheme}>{isDarkMode ? <FaMoon /> : <FaSun />}</p>
            </div>
          </div>
        </div>
        ...
        <header
          id="header-2"
          className={`${
            isDarkMode ? "bg-slate-700  text-white" : "bg-slate-200 text-black"
          }    w-full shadow-md`}
        >
          <div className="container w-full  flex justify-start items-center p-1 px-5">
            <div className="flex  ">
              <div className="hidden sm:flex ml-1 space-x-6">
                ...
                <Link
                  className="hover:text-yellow-500 cursor-pointer"
                  to="/aaminshop/aboutus"
                >
                  About
                </Link>
                <Link
                  className="hover:text-yellow-500 cursor-pointer"
                  to="/aaminshop/contact"
                >
                  Contact
                </Link>
              </div>
         ...
         <button
            ...
            <nav
              className={`sm:hidden fixed top-24 md:items-start mt-1  z-300 rounded-md justify-start ${
                isDarkMode ? "bg-slate-900 " : "bg-slate-400"
              }  md:space-x-6 ${isOpen ? "block" : "hidden"}`}
            >
              <Link
                to="/aaminshop/"
                className="block py-2 px-4 hover:bg-slate-800 hover:text-white rounded"
              >
                Home
              </Link>
              <Link
